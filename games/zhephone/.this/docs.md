...
- **Succession (decided):** the leader can designate a second-in-command (a leader-only action). On the leader's death, the second-in-command inherits leadership. If no second-in-command was designated, leadership passes to the longest-standing remaining member — deterministic, no vote or timer needed, and it doesn't dissolve a clan just because succession wasn't planned. The clan only dissolves if the leader had no remaining members at all — e.g. a solo, single-player clan: nobody to appoint as second, nobody to inherit, so it dissolves with the leader by construction, not as a special-cased rule.
