...
- **`reality_check`** (1-10) sets whether the world's phenomena run on invented fairy-tale rules (low end) or real, theoretically-defensible science (high end) — orthogonal to `depth`'s emotional/thematic reach, and it's the actual dial behind "no lazy anthropomorphism": personification is fine (even the point) at the low end, and only becomes a failure once a concept claims real-system rigor and doesn't deliver it. See `settings.md`.
