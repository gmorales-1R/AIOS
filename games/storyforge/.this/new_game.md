...
1. **Take the idea as-is.** Whatever the player says ("dwarves mining who find a stone golem") is the fiction. Don't negotiate the wrapper.
   Then set up **`settings.json`** (copy `storyforge/settings.default.json`, override per `settings.md`): confirm `locale`, keep the **narrator neutral** (style it only if asked), then rating, player count/ages, difficulty, depth, ideal duration, learning intensity. Character voices are *not* set here — they're derived per character from background (step 7 / the `Voice` fields). If the player didn't specify, keep defaults and mention what you assumed; don't stall the game to interrogate them.
...
4. **Plant the premise you'll overturn.** Decide the Act IV **turn** (the reveal/betrayal/twist — see `depth` in `settings.md` for how hard it should recontextualize things) and the belief you'll set up in Act II so there's something real to break. (Anti-shallow: the answer must not be visible from turn 1.)
5. **Read the players' level** from context / the returning `players.md`, and set scaffolding accordingly.
6. **Create the match node:** `matches/<slug>/settings.json` (from step 1), `this.md`, `.this/{players,lore,narrative}.md` (from the templates in `turn_engine.md`), `sessions/`. Seed `lore.md` with the concept + ladder; seed `narrative.md` with rung 1 active.
7. **Cast the voices.** Give every character (crew + key NPCs) a distinct **`Voice`** derived from their background (role/class/era/region), drawing on the `locale` palette. Record it in `players.md`/`lore.md`. This is what makes the RP sing — do it before the first line of dialogue.
8. **Add the match** to `storyforge/this.md`'s Matches table.
...
- `settings.json` created, narrator neutral, each character given a distinct **`Voice`** by background, open decisions? ✅

## Reusing a cast

A returning troupe (e.g. the *Estrella Errante* crew: Clara/Guille/Maida/José) can headline a new space game — copy their character blocks into the new match's `players.md`. A different setting (dwarves, deep sea) usually wants a fresh cast; that's fine. Each game is a blank slate otherwise — no player history carries over between matches.
